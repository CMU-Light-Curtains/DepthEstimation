...
std::map<std::string, std::vector<float>> evaluateErrors(std::vector<std::vector<float>>& errors_out){
    ...
    for (int32_t i = 0; i < errors_out[0].size(); i++) {
      ...
      results[std::string(metrics[i])] = {statMean(errors_out, i), statMin(errors_out, i), statMax(errors_out, i)};
    }
    return results;
}
