...
\smallskip
We wish to estimate the depth map $\DD = \{d_{u, v}\}$ of the scene, which specifies the depth value $d_{u, v}$ for every camera pixel $(u, v)$. This is represented as a tensor of some fixed resolution [320x240] with each pixel in image $\DD$ encoding the depth value $d_{u,v}$. Since there is inherent ambiguity in the depth value at every pixel, we output a \textit{probability distribution} over depths for every pixel. Let us define $\dd_{u,v}$ to be a \textit{random variable} for depth predictions at the pixel $(u, v)$. as a bernoulli distribution $P(\mathbf{d}(u,v))$. $\D$ represents a set of $N$ quantized, uniformly-spaced depth values for each pixel that lie within $(d_{min}, d_{max})$. In this work, we use $N=64$, resulting in a Depth Probability Volume (DPV) tensor of size [64, 240, 320]
\begin{align}
   &\D=\left\{ d_{0}, \dots, d_{n}\right\}; d_{i}=d_{min}+(d_{max}-d_{min}) \cdot t 
   \label{eq:d_candi}
   \\
